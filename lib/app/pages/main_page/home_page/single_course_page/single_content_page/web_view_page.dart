  ...
  Widget build(BuildContext context) {
    return directionality(
      child: OrientationBuilder(builder: (context, orientation) {
        return Scaffold(
          appBar: appbar(title: title ?? '', context: context),
          body: isShow
              ? InAppWebView(
                  onJsBeforeUnload:
                  ...
                  shouldOverrideUrlLoading:
                    ...
                    if (isSendTokenInHeader) {
                      if (!(navigationAction.request.headers
                        ...
                        if (navigationAction.request.headers != null) {
                          navigationAction.request.headers?.addAll({
                            "Authorization": "Bearer $token",
                          });
                          controller.loadUrl(
                    ...
                    var header = {
                      ...
                      'x-api-key': Constants.apiKey,
                      'x-locale':
                          locator<AppLanguage>().currentLanguage.toLowerCase(),
                    };

                    if (navigationAction.request.headers == null ||
                        (navigationAction.request.headers?.isEmpty ?? true)) {
