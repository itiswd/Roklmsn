...
class QuizWidget{
  ...
  static Widget multiAnswerItem(Answer answer,bool isReview, {UserAnswer? userAnswer, backgroundColor = Colors.white, required BuildContext context}){
      ...
      decoration: BoxDecoration(
        ...
        border: isReview
          ...
          color: userAnswer == null
              ? Theme.of(context).colorScheme.onSurface // بدلاً من Colors.white
              : (userAnswer.answer.toString() != answer.id.toString())
              ? Theme.of(context).colorScheme.onSurface // بدلاً من Colors.white
              : (userAnswer.status ?? false)
              ? Theme.of(context).colorScheme.primary // بدلاً من mainColor()
              : Theme.of(context).colorScheme.error, // بدلاً من red49
          width: 4.2,
        )
            : Border.all(
          color: answer.isSelected
              ? Colors.green// بدلاً من mainColor()
              : Theme.of(context).colorScheme.onSurface.withOpacity(0), // بدلاً من Colors.white.withOpacity(0)
          width: 5.2,
        ),
              ...
              child: Text(
                ...
                style: Theme.of(context).textTheme.bodyMedium?.copyWith(
                  color: Colors.white
                    ...
                    //     ? userAnswer?.answer?.toString() == answer.id.toString()
                    //     ? (userAnswer?.status ?? false)
                    //     ? Theme.of(context).colorScheme.primary // بدل mainColor()
                    //     : Theme.of(context).colorScheme.error // بدل red49
                    //     : answer.image == null
                    //     ? Theme.of(context).colorScheme.onSurface.withOpacity(0.6) // بدل greyA5
                    //     : Theme.of(context).colorScheme.onPrimary // بدل Colors.white
                    //     : answer.isSelected
                    //     ? Theme.of(context).colorScheme.onPrimary // بدل mainColor()
                    //     : answer.image == null
                    //     ? Theme.of(context).colorScheme.onSurface.withOpacity(0.6) // بدل greyA5
                    //     : Theme.of(context).colorScheme.onPrimary // بدل Colors.white
                ).copyWith(
            ...
            if(isReview && ((answer.correct ?? 0) == 1))...{
              PositionedDirectional(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    color: Theme.of(context).colorScheme.primary, // بدل mainColor()
                    borderRadius: borderRadius(),
                  ),
                  child: Text(
                    appText.correct,
                    style: style10Regular().copyWith(
                      color: Theme.of(context).colorScheme.onPrimary, // بدل Colors.white
                      height: 1,
                    ),
                  ),
                ),
