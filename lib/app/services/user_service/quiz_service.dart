...
class QuizService {
    ...
    if (jsonResponse['success']) {
      for (var j = 0; j < jsonResponse['data']['quizzes'].length; j++) {
        ...
        if (jsonResponse['data']['quizzes_results'] != null) {
          try {
            for (var i = 0; i <
                jsonResponse['data']['quizzes_results'].length; i++) {
              if (jsonResponse['data']['quizzes_results'][i]['quiz_id'] ==
                  jsonResponse['data']['quizzes'][j]['id']) {
                counter++;
                userGradeSum += int.tryParse(
                    jsonResponse['data']['quizzes_results'][i]['user_grade']
                        ?.toString() ?? '0') ?? 0;
              }
            }
          } catch (e) {}
  ...
  static Future<List<QuizModel>> getStudentResults() async {
    ...
    try {
      String url = '${Constants
          .baseUrl}panel/quizzes/results/my-student-result';
  ...
  static Future<List<Quiz>> getNotParticipated() async {
    ...
    try {
      ...
      print(res.body);
      print("res.body getNotParticipated");
      var jsonResponse = jsonDecode(res.body);
      print(jsonResponse);
      print("jsonResponse");
      if (jsonResponse['success']) {
  ...
  static Future<Map?> startQuiz(int quizId) async {
    try {
      ...
      log("jsonResponse startQuiz");


      if (jsonResponse['success']) {
        log(jsonResponse.toString());
        log("jsonResponse Start Quiz");
        return {
        ...
        if (jsonResponse['status'] == "max_attempt") {
  ...
  static Future<QuizModel?> reviewQuiz(int quizId, String beforTab) async {
    try {
      String url = beforTab == 'StudentResults' ? '${Constants
          .baseUrl}panel/quizzes/results/$quizId' : '${Constants
      ...
      if (jsonResponse['success']) {
        ...
        ErrorHandler().showError(
            ErrorEnum.error, jsonResponse, readMessage: true);
        return null;
      }
    } catch (e) {
      return null;
  ...
  static Future<bool> storeResult(int quizId, int quizResultId,
  ...
  static Future<bool> storeReview(int quizResultId,
      List<Question> questions) async {
    try {
      String url = '${Constants
          .baseUrl}panel/quizzes/results/$quizResultId/review';
